...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.fill_opacity_slider.setValue(50)
        fill_opacity_row.addWidget(self.fill_opacity_slider)
        self.fill_opacity_value_label = QLabel("50%")
        ...
        self.fill_color_button.colorChanged.connect(self._refresh_preview)
    ...
    def _apply_saved_selection(self, saved: dict, group_infos: list[tuple[str, int]]) -> None:
        ...
        if "island_fill" in opts:
            # Set last: toggling this triggers _on_island_fill_toggled,
            # which shows/hides the fill controls and refreshes the
            # preview, so it should fire after the fill color/opacity
            # values above are already in place.
            self.island_fill_check.setChecked(opts["island_fill"])
